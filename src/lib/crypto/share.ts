...
export async function deriveShareKey({
  ...
  const materialInput = password
    ? new TextEncoder().encode(`${bytesToHex(token)}:${password}`)
    : token;
  const material = await crypto.subtle.importKey(
    "raw",
    toBufferSource(materialInput),
    "PBKDF2",
    false,
    ["deriveKey"],
  );
...
export async function hashTokenHex(token: Uint8Array): Promise<string> {
  const digest = await crypto.subtle.digest("SHA-256", toBufferSource(token));
  return bytesToHex(new Uint8Array(digest));
}

export function newPasswordSalt(): Uint8Array {
  return randomBytes(SALT_LENGTH);
}

export function newShareToken(): Uint8Array {
